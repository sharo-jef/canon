...
- `@name`: フィールド名を指定するためのアノテーション
- `@fFlatten`: フラット化を指定するためのアノテーション
- `@deprecated`: 非推奨の要素に対して使用するアノテーション
- `@serialize`: シリアライズ時に特定の形式で出力するためのアノテーション
  - メソッドか getter に付与する
  - `@serialize 'methodName'` の形式で、シリアライズ時に特定のメソッド or getter を呼び出すことができる
  - 一つの struct に複数の `@serialize` を付与することはできない
...
- `#schema` ディレクティブを使用しているファイルで `schema` を定義することはできない
  - `#schema` ディレクティブは、スキーマファイルを参照するためのものであり、スキーマの定義自体は別のファイルで行う必要がある
